(function ($) {
	$.widget("ui.rcarousel", {
		_create: function() {
			...
			$root
				.addClass(Â "ui-carousel"Â )
				.children()
				.wrapAll(Â "<div class='wrapper'></div>" );
		...
		_addElement: function( jQueryElement, direction )Â {
			var $root = $( this.element ),
				$content = $root.find(Â "div.wrapper"Â ),
				options = this.options;
		...
		_autoMode: function( direction ) {
			var options = this.options,
				data = $( this.element ).data(Â "data"Â );
		...
		_generatePages: function() {
			...
			function _paginate() {
				...
				while ( !_islastPage(data.pages[data.pages.length - 1]) || _isBeginning ) {
					...
					// when we run ouf of elements (_end - _start <Â _visible) we must add the difference at the begining
		...
		_goToPrevPage: function( by ) {
			...
			$( _page ).each(
				function( i, el )Â {
					if ( el.get(0) === $(_oldPage[i]).get(0) )Â {
						_theSame = true;
					} else {
						_theSame = false;
					}
			...
			if ( data.appended && _theSame )Â {
				if ( data.pageIndex === 0 )Â {
		...
		_goToNextPage: function( by ) {
			...
			if ( options.orientation === "horizontal" )Â {
		...
		_saveElements: function()Â {
			var $el,
				$root = $( this.element ),
				$elements = $root.find( "div.wrapper"Â ).children(),
				data = $root.data( "data" );
